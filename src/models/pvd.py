...
class PVDModel(BaseSteganographyModel):
    ...
    @staticmethod
    def compute_pvd(neighbors: List[np.ndarray]) -> int:
        """Compute the pvd (difference between pixels)."""

        return max(neighbors) - min(neighbors)

    @staticmethod
    def classify(pvd: int) -> int:
    ...
    @staticmethod
    def modify_pixel(pixel: np.ndarray, nbits: int, text_to_hide: str) -> np.ndarray:
    ...
    @staticmethod
    def get_neighbors(i: int, j: int, channel_img: np.ndarray) -> List[int]:
